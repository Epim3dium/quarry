...
#define STEAM_MIN_AGE_TO_CONDENSATE 90
    ...
    CellVar::properties[eCellType::Dirt] = CellConstants(
        ...
        0.f,
        //behaviour
        CellVar::properties[eCellType::Cobblestone].update_behaviour,
        //colors
    ...
    CellVar::properties[eCellType::Seed] = CellConstants(
        ...
        [](vec2i v, Grid& grid) {
            if(grid.get(v.x, v.y - 1).type == eCellType::Dirt) {
                ...
                t.var.Wood.Dir.y = 1;
                t.var.Wood.Dir.x = 0;
                t.var.Wood.BranchCount = 0;
                t.var.Wood.AvGrowthLen = CellVar::rng.Random<unsigned char>(WOOD_MIN_BRANCH_SIZE, WOOD_MAX_BRANCH_SIZE) + WOOD_MAX_BRANCH_SIZE;
                grid.set(v.x, v.y, t);
            }
            else {
                CellVar::properties[eCellType::Sand].update_behaviour(v, grid);
        ...
        [](vec2i v, Grid& grid) {
            ...
            if(me.var.Wood.AvGrowthLen > 0) {
                ...
                if(is_replaceable(v.x + me.var.Wood.Dir.x, v.y + me.var.Wood.Dir.y)) {
                    ...
                    t.var.Wood.AvGrowthLen = me.var.Wood.AvGrowthLen - 1;
                    t.var.Wood.BranchCount = me.var.Wood.BranchCount;
                    grid.set(v.x + t.var.Wood.Dir.x, v.y + t.var.Wood.Dir.y, t);

                    bool growingSideways = me.var.Wood.Dir.x != 0;
                    if(me.var.Wood.BranchCount < 5 && me.var.Wood.AvGrowthLen < WOOD_MAX_BRANCH_SIZE - WOOD_MIN_BRANCH_SIZE && CellVar::rng.Random() < WOOD_MUTATE_CHANCE) {
                        {
                            auto t = me;
                            t.var.Wood.BranchCount += 1;
                            grid.set(v, t);
                        }

                        CellVar t(eCellType::Wood);
                        t.var.Wood.AvGrowthLen = CellVar::rng.Random(WOOD_MIN_BRANCH_SIZE, WOOD_MAX_BRANCH_SIZE);
                        t.var.Wood.BranchCount = me.var.Wood.BranchCount;
                        if(growingSideways) {
                            t.var.Wood.Dir.y = 1;
                            t.var.Wood.Dir.x = 0;
                        ...
                        t.var.Wood.AvGrowthLen = -1;
                        grid.set(v, t);
                    }
                }
                //if it is at max growth length
            } else if(me.var.Wood.AvGrowthLen != -1) {
                auto t = CellVar(eCellType::Leaf);
                t.var.Leaf.CurGrowthLen = 5;
                grid.set(v, t);
            }
...
#define LEAF_DUPE_CHANCE 0.5f
    CellVar::properties[eCellType::Leaf] = CellConstants(
        ...
        [](vec2i v, Grid& grid) {
            ...
            if(me.var.Leaf.CurGrowthLen > 0) {
                auto t = me;
                t.var.Leaf.CurGrowthLen -= 1;
